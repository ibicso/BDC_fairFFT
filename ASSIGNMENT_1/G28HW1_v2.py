...
def fair_fft_sequential(Ka, Kb, points):
    ...
    #coordinates and labels into NumPy arrays so we do not use loops
    ...
    min_distances = np.sum((coords - coords[idx]) ** 2, axis=1)
    #using total len so we always return the desired num of centers even if for one class we do not have enough
