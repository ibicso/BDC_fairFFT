...
def fair_fft_sequential(Ka, Kb, points):
    ...
    min_distances = np.sum(
        (coords - coords[idx]) ** 2, axis=1
    )  # fast with np and no need to sqrt since we only compare distances
...
def map_reduce_fair_fft_profiling(rdd, Ka, Kb, L):
    ...
    # 2. Wrapper function to dynamically profile Round 1 partitions
    def round1_wrapper(partition_idx, partition_iterator):
        points = list(partition_iterator)
        if not points:
            return []
        log_filename = f"logs/mem_profile_R1_P{partition_idx}.log"
        os.makedirs(os.path.dirname(log_filename), exist_ok=True)
        with open(log_filename, "a") as f:
            f.write(f"\n--- Profiling Round 1 | Partition {partition_idx} ---\n")
            # Dynamically apply profile to stream directly into this specific file
            profiled_func = profile(stream=f)(fair_fft_sequential)
            return profiled_func(Ka, Kb, points)

    # 3. Use mapPartitionsWithIndex to get the partition index
    local_coresets_rdd = rdd.mapPartitionsWithIndex(round1_wrapper)

    global_coreset = local_coresets_rdd.collect()

    # 4. Profile Round 2 to a dedicated driver log file
    log_filename_r2 = "logs/mem_profile_R2_Driver.log"
